...
from src.nets import UMobileNet, DeepLabV3, DeepLabHeadV3Plus
from src.utils.special_modules import Bottleneck, IntermediateLayerGetter, ResNetBottleneck
...
class GRContrailsClassifierModule(pl.LightningModule):
    def __init__(self, image_size: int, in_image_channels: int, optimizer: optim.Optimizer):
        ...
        self.train_dice = DiceScore(num_classes=2, input_format='index')
        self.val_dice = DiceScore(num_classes=2, input_format='index')
    ...
    def training_step(self, batch, batch_idx):
        ...
        dice = self.train_dice(output_masks.softmax(dim=1).argmax(dim=1).to(torch.long), masks.squeeze().to(torch.long))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        accuracy = self.accuracy(output_masks, masks)
        loss = self.loss(output_masks, masks)
        dice = self.val_dice(output_masks.softmax(dim=1).argmax(dim=1).to(torch.long), masks.squeeze().to(torch.long))
        # Update accuracy and loss.
...
class GRContrailBinaryClassifier(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_dice(output_masks, seg_masks.to(torch.long))
...
class GRContrailsDeepLabResNet50(pl.LightningModule):
    def __init__(self, optimizer: torch.optim, output_stride=8, aspp_dilate=[12, 24, 36], inplanes=2048, low_level_planes=256,
        ...
        model = ResNetBottleneck(Bottleneck, [3, 4, 6, 3], replace_stride_with_dilation=[False, True, True])
